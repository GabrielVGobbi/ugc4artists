...
export const getPageTitle = (pathname: string): { title: string; subtitle: string } => {
    ...
    if (pathname.startsWith('/app/campaigns/') && pathname !== '/app/campaigns/create') {
        if (pathname.endsWith('/pay')) {
            return {
                title: 'Pagamento da Campanha',
                subtitle: 'Finalize o pagamento da campanha selecionada.',
            };
        }

        return {
            title: 'Detalhes da Campanha',
            subtitle: 'Visualize informações da campanha selecionada.',
        };
    }

...
trait SearchableTrait
{
    ...
    {
        $query = (array)($query ?: array_not_null(Request::except(['page', 'pageSize', 'order', 'sort', 'per_page'])));
            ...
            if ($field == 'search' && !empty($value)) {
                ...
                $builder->where(function (Builder $builder) use ($searchable, $value) {
                    foreach ($searchable as $key) {
                        if (str_contains($key, ':')) {
                            // 🚀 Cria constraint e aplica via interceptor
                            $constraint = \App\Supports\Traits\Searchable\Constraint::make($value);
                            $this->applyConstraint($builder, $key, $constraint, 'or');
                        } else {
                            // Campos simples continuam com LIKE direto
